...
def write_data(df):
    df["ts_code"] = df["ts_code"].str.slice(stop=6)
    res = df.to_sql('stock_deal', engine_ts, index=False, if_exists='append', chunksize=5000, index_label='id')
    return res
...
def record_average(start_day):
    print("开始计算日平均数...start_day:{}".format(start_day))
    # 先不计算科创板
    sql = """SELECT symbol FROM stock_basic WHERE (symbol > 000000 and symbol < 300000) or (symbol > 600000 and symbol < 680000)"""
    base_df = pd.read_sql_query(sql, engine_ts)
    interval_day = date_util.get_interval_day(start_day)

    for _, base_row in base_df.iterrows():
        # 每次取只要取几十条数据即可
        sql = "SELECT * FROM stock_deal WHERE ts_code = '{}' ORDER BY trade_date DESC limit {}".format(base_row['symbol'], interval_day + 20)
        df = pd.read_sql_query(sql, engine_ts)
        ...
        for index, row in df.iterrows():
            ...
            if index < len(average_data['20_day_ma']) and index < len(average_data['10_day_ma']) and index < len(average_data['5_day_ma']):
                if row['avg_5'] == 0 or row['avg_5'] is None:
                    avg_5 = round(average_data['5_day_ma'].iloc[index], 2)
                if row['avg_10'] == 0 or row['avg_10'] is None:
                    avg_10 = round(average_data['10_day_ma'].iloc[index], 2)
                if row['avg_20'] == 0 or row['avg_20'] is None:
        ...
        if len(update_data) >= 1:
            cursor = conn.cursor()
            sql = "update stock_deal set avg_5 = %s, avg_10 = %s, avg_20 = %s where id = %s"
